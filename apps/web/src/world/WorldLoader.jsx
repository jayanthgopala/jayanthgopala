...
// Preloader tracking world build steps and asset readiness
export default function WorldLoader({ ready, steps = [], name = '' }) {
  const [opening, setOpening] = useState(false);
  const [gone, setGone] = useState(false);
  const barRef = useRef(null);
  const pctRef = useRef(null);
  const shown = useRef(0); // Peak progress to avoid backwards movement
  ...
  // Progress animation during active step
  useEffect(() => {
  ...
  // Complete progress and trigger opening transition
  useEffect(() => {
    if (!ready) return undefined;
    shown.current = 1;
    write(1);
